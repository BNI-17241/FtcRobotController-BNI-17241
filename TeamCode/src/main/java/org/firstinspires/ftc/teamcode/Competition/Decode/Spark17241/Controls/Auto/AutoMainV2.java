...
public abstract class AutoMainV2 extends OpMode {
    ...
    protected void updateFlywheelAndGate() {
        ...
        // Update nominal only when not arming/boosting (prevents drift)
        if (scoringState == scoreState.IDLE) {
        ...
        // Track consecutive stable-in-gate frames
        if (inGate) {
            gateStableCount++;
        } else {
            gateStableCount = 0;
        ...
        switch (scoringState) {
            ...
            case WAIT_FOR_GATE:
                ...
                if (inGate && gateStableCount >= gateStableNeeded && interShotOk) {
                    ...
                    nominalTarget = targetVelocity;
                    // Apply boost (or nominal if boostFactor==1.0)
            ...
            case FEEDING:
                ...
                if (timer.milliseconds() >= feedMs) {
                    ...
                    lastShotMs = shotClock.milliseconds();
                    timer.reset();
            ...
            case RECOVERING:
                gateStableCount = 0; // must settle and re-qualify
    ...
    protected void baseTelemetry() {
        ...
        telemetry.addData("Gate Stable", "%d/%d", gateStableCount, gateStableNeeded);
        telemetry.addData("InterShot(ms)", "%d/%d", (int)(shotClock.milliseconds() - lastShotMs), (int)minInterShotMs);
    }
